...
def run_training_step_with_backprop(src_batch, tgt_batch, parameter_list, model_params, optimizer_state, step_number, config):
    ...
    total_loss = compute_batch_training_loss(src_batch, tgt_batch, model_params, config)
    total_loss.backward()
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    ...
    new_column = token_ids.view(-1, 1)
    return torch.cat([parent_rows, new_column], dim=1)
...
def mark_finished_beams(token_ids, finished_flags, end_token_id):
    ...
    finished_flags = finished_flags | (token_ids == end_token_id)
...
def select_best_finished_beam(finished_sequences, finished_scores, alpha):
    ...
    adj_scores = [score/compute_length_penalty(len(seq), alpha) for seq,score in zip(finished_sequences, finished_scores)]

    idx = torch.argmax(torch.tensor(adj_scores))
    return {'score':adj_scores[idx],'sequence': finished_sequences[idx]}
